//! Wasm entry for the tree example. Zunk owns the rAF loop; this file
//! exports `init` / `frame` / `resize`. No text input, so no char/key
//! routing and no cursor-blink focus tracking.
...
export fn init() void {
    ...
    host = Host.init("Teak — Tree", 720, 600) catch unreachable;
...
export fn frame(_: f32) void {
    ...
    teak.buildVertices(&verts, alloc, cur_cmds, rects_store[cur][0..cur_cmds.len], transient_state);
